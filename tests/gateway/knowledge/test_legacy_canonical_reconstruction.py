...
def test_reconstructs_existing_nodes_with_fact_source_and_one_speaker_only(
    ...
    assert _rows(target, "SELECT COUNT(*) FROM memory2_facts") == [(3,)]
    assert _rows(target, "SELECT COUNT(*) FROM knowledge_statement_sources") == [(3,)]
    assert _rows(target, "SELECT COUNT(*) FROM memory2_fact_sources") == [(3,)]
    assert _rows(
        target,
