...
@pytest.mark.asyncio
async def test_slash_approve_command_sends_to_target_chat(tmp_path: Path) -> None:
    tools, store, log = _tools(tmp_path, opt_in_group=True)
    proposal = await tools.propose_speakup(
        chat_id="group@g.us",
        message="hello group",
    ...
    await tools.commit_speakup(str(proposal["proposal_id"]))
    preview = await tools.bus.consume_outbound()
    command = preview.content.split("Approve: ", 1)[1].splitlines()[0].strip()

    assert command.startswith("/spk approve ")
    ...
    next_fn = AsyncMock()
    await middleware(_owner_ctx(command), next_fn)
    outbound = await tools.bus.consume_outbound()
...
@pytest.mark.asyncio
async def test_slash_deny_command_prevents_send(tmp_path: Path) -> None:
    tools, store, log = _tools(tmp_path, opt_in_group=True)
    proposal = await tools.propose_speakup(
        chat_id="group@g.us",
        message="hello group",
    ...
    await tools.commit_speakup(str(proposal["proposal_id"]))
    preview = await tools.bus.consume_outbound()
    command = preview.content.split("Deny: ", 1)[1].splitlines()[0].strip()

    assert command.startswith("/spk deny ")
...
@pytest.mark.asyncio
async def test_speakup_slash_alias_denies(tmp_path: Path) -> None:
    tools, store, log = _tools(tmp_path, opt_in_group=True)
    proposal = await tools.propose_speakup(
        chat_id="group@g.us",
        message="hello group",
    ...
    next_fn = AsyncMock()
    await middleware(
        _owner_ctx(f"/speakup deny {proposal['proposal_id']}"),
        next_fn,
    )

    assert tools.bus.outbound_size == 1  # preview remains queued on the test bus
    history = await log.history("whatsapp", "group@g.us", limit=5)
    assert history[0]["status"] == "denied"
    assert next_fn.await_count == 0
