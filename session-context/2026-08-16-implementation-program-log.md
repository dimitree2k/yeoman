...
### 2026-08-16 — Incident containment Task 3b reconnect skill accepted

- The installed `/home/dm/.codex/skills/yeoman-whatsapp-reconnect` contract now pins migration-toolkit commit `6ba55014242953e72ec7a29e75041f704452b885` and helper SHA-256 `0de5b3df3d7751aebdbe4c2255e2390ae5e119b7df79a611b327c30b16f27154`; it forbids silent fallback to the older source checkout.
- The skill requires `yeoman-runtime` first and three distinct owner gates: phone revocation of every linked device, explicit active-auth quarantine authorization, and phone readiness to scan. It separates receipt-backed encrypted non-restorable auth quarantine from helper `start --owner-revoked-all-devices`, preserves memory/messages/archive state, limits lifecycle control to Bridge, and prohibits restore of compromised auth.
- Completion guidance requires changed session identity/fingerprint, owner-confirmed linked-device inventory, send/receive provenance receipts, and post-relink memory/archive integrity comparison; `connected=true` alone is insufficient. Provider transcript-retention uncertainty and same-disk recovery remain explicit residual risks.
- The obsolete installed wrapper was deleted because it could select the older source checkout and print path/error details. Accepted installed hashes: `SKILL.md` `3ad196af7cfe4c417410a3fff78bdb1616f7e94cbc9efbcac551e1aeabec6df3`, `agents/openai.yaml` `c6815e9ee9f1e648f1070feed46fce894188232ef552a82221356a4bb7741e1b`, and controlled contract test `3fc698099f79c8aac482bdae4f5266443d843cfea2135f87e0517f614b1145cd`.
- Fresh controller contract test and skill validation passed. Scoped Terra review returned ACCEPT with no Critical/Important defect or capability overclaim. No live helper, test-connected runtime, QR, service, auth, message, memory, archive, or receipt operation occurred. Task 4 remains unauthorized pending the major-turn Luna reviews and explicit owner action.

### 2026-08-16 — Major-turn Luna review after Tasks 3 and 3b

- Agent A and Agent B reviewed the durable package in their persistent threads with explicit `gpt-5.6-luna` and high reasoning, bound to source ledger `53ec391`, toolkit range `686a1ee84f060b4bcb5acd35b65887cbafed2df3..6ba55014242953e72ec7a29e75041f704452b885`, helper SHA-256 `0de5b3df3d7751aebdbe4c2255e2390ae5e119b7df79a611b327c30b16f27154`, and installed skill SHA-256 `3ad196af7cfe4c417410a3fff78bdb1616f7e94cbc9efbcac551e1aeabec6df3`.
- Both accepted Tasks 3/3b and found the previous helper/skill boundary blockers closed. Agent B returned GO to request the three owner confirmations but only a conditional execution GO after a fail-closed quarantine/evidence controller exists. Agent A returned NO-GO for owner interaction until those controller-side mechanisms are implemented and demonstrated. The stricter gate governs; no owner confirmation is requested yet.
- Pre-owner implementation requirements are: synthetic-tested dual-recipient encrypted non-restorable auth quarantine with verified service stop, atomic ciphertext, signature/decryption checks, plaintext absence, empty owner-only active auth, and allowlisted receipt commitments; a non-disclosing keyed pre/post session fingerprint; a protected owner-attestation/device-inventory evidence format; a deterministic owner-only no-retry send/receive smoke receipt; and a domain-aware comparator against manifest commitment `6ae6476466c728bbb423be16182b6341e47e38c0d55073f9831751452346f833` that preserves unsolicited inbound messages and fails on unexplained changes.
- Required live order after those mechanisms pass review: verify pinned hashes and service state; obtain separate owner all-device-revocation confirmation; obtain explicit quarantine authorization; execute and verify quarantine; obtain phone-ready confirmation; run only the pinned Bridge-only helper; verify changed identity and intended device inventory; perform one controlled send/receive smoke; reconcile protected state; clean QR; produce signed/encrypted incident-close evidence; obtain final Luna A/B GO.
- HIGH / SEV-2 remains until revocation and fresh-session verification. Provider transcript retention, historical confidentiality, same-disk recovery, and sensitivity of the encrypted old-auth quarantine remain explicit residual risks. Bridge and Overseer remain inactive; Gateway active; no auth, QR, memory, message, archive, or receipt state changed during review.

### 2026-08-16 — Task 4 pre-owner implementation plan accepted

- The four-part pre-owner plan bundle now separates common protected evidence, crash-safe non-restorable auth quarantine, quiesced v1/v2 state proof, and direct Bridge observer/one-shot smoke. Accepted plan hashes are recorded in `.superpowers/sdd/2026-08-16-yeoman-whatsapp-session-incident-containment/task-4-prep-plan-review.md`.
- A fresh Terra reviewer initially rejected active-Gateway/WAL races, positional SQLite comparison, unauthenticated opaque owner sources, an unjournaled auth-exchange crash window, and process-local-only send retry prevention. Correction rounds replaced those boundaries with full quiescence, stable-key row equality, exact host thread/turn/message references, PREPARED transaction recovery, and durable one-shot reservation.
- Final re-review returned ACCEPT with no remaining Critical or Important issue after one versioned `canonical_auth_tree_v1` became mandatory for old/current/pre-send identity and `capture_failed` became a terminal incident-close NO-GO.
- The first future Luna code GO authorizes only stopping Bridge/Gateway/Overseer and real read-only v1/v2 capture. A second evidence-bound Luna GO is required before any owner confirmation or auth/QR/send action. Terra remains authorized for implementation/task reviews; standing Agent A/B remain Luna/high.
- No service, QR, linked device, authentication, memory, raw-message, archive, receipt, or operator-evidence state changed during plan production/review. Bridge and Overseer remain inactive; Gateway active; owner interaction remains NO-GO.

### 2026-08-16 — Task 4 common protected-evidence module accepted

- A Terra implementer delivered the common evidence primitive on the migration-toolkit branch in commits `9dfaa70190e94404c3b49e2032f938284b73be19`, `729d74393e9f6dd30a14e828e3c8e740e384c86f`, and final `de462af3dc1fbbbcdd212029374f0c343fb00baf`.
- The primitive provisions the dedicated incident HMAC key only through an explicit operation, domain-frames and streams bounded plaintext directly into dual-recipient age encryption, verifies both decryptions and the detached SSH signature, publishes descriptor-relative artifacts atomically, and journals exact host-turn owner records for crash-safe retry/refusal semantics.
- Two security-fix rounds closed raw provisioning failures, descriptor/child cleanup, bounded capture, post-rename directory durability, strict journal corruption handling, concurrent source locking, and owned-versus-borrowed crypto lifecycle. The same Terra security reviewer re-read the full final code/tests and returned ACCEPT with no Critical or Important findings.
- Fresh controller verification from the committed toolkit tree passed 18 focused tests, Ruff, and `git diff --check`; the toolkit worktree was clean. No real service, auth tree, runtime state, program key, evidence root, QR, message, memory, archive, or receipt was accessed or changed. Owner interaction and all live rotation actions remain NO-GO.

### 2026-08-16 — Task 4 auth-quarantine and identity module accepted

- The migration-toolkit implementation through `68839a3` now provides crash-safe, non-restorable authentication quarantine; protected gate/transaction/receipt journals; canonical old/current auth-tree identity; and a fail-closed fixed production quiescence probe. It preserves an empty owner-only active auth directory and exposes public-safe commitments only.
- Eight review/fix rounds closed durability, exact exchange-window recovery, evidence-gate alignment, cleanup provenance, gate-before-probe ordering, endpoint and systemd allowlisting, bounded child lifecycle, and final kill-path reaping. The last mutation removes the unconditional post-kill wait and is detected by an explicit `[0.2, None]` lifecycle assertion.
- The same independent Terra security reviewer re-read the full final common/quarantine implementation and returned ACCEPT with no Critical or Important issue. Independent and controller verification each passed 106 focused tests without warnings, scoped Ruff, and `git diff --check`; the toolkit worktree was clean. Broader Ruff still reports unrelated pre-existing I001/F401 findings in `tests/shared/test_tracing.py`, which are not part of this acceptance.
- No production adapter, live service/socket/port probe, authentication tree, program key, operator-evidence root, QR, message, memory, archive, or receipt was accessed or mutated. Owner interaction and all live rotation actions remain NO-GO; the next bounded implementation task is the v1-derived state-evidence module.

### 2026-08-16 — State-evidence legacy-v1 contract correction pending review

- The state implementer stopped before RED because the accepted common reader requires a new-format `EvidenceCommitment`, while the pre-rotation v1 manifest is a legacy external age artifact and the plan originally pinned only its plaintext SHA-256. No toolkit edit or live action occurred.
- A controller metadata-only lookup plus a filtered read of the historical artifact-creation tool call established the omitted fixed artifact names/sizes, exact v1 JSON schema/counts/root labels, and public SSH signature envelope without decrypting v1 or opening any identity. `workspace_persona_evolution` is preserved only as inert history and does not restore omitted target behavior.
- The first independent Terra plan review rejected the proposed compatibility boundary until it gained a separate legacy crypto path, held-FD hash-to-consumption binding, exact cumulative caps, identity-to-recipient checks, explicit legacy types, and child/FD/no-output tests. The corrected plan adds those controls, distrusts the adjacent commitments file, and freezes new-format HMAC/ciphertext-signature semantics unchanged. Implementation stays paused for re-review; all live and owner actions remain NO-GO.
- The same reviewer accepted the corrected plan at SHA-256 `ff993684668bec71b11078e1f9718ef9f4883871203a7914f5ec3fe144418080` with no remaining Critical or Important issue. State implementation may resume with synthetic fixtures only; production v1 decryption/capture still waits for the first standing Luna code GO.
- The resumed implementer stopped again because the plan's logical SQLite registry was unspecified. Two independent Terra reviews accepted deleting that redundant boundary: complete v1/live/pre/post path-set and byte/metadata equality is strictly stronger here because no v1-root growth is allowed. The corrected design uses one exact adapter for DB/WAL/SHM/journal, JSONL, and every opaque file; only the authenticated observer-evidence chain may grow. Logical target import remains a later migration receipt, not part of incident preservation.
- Final plan review required four more explicit boundaries before implementation: fixed label-to-absolute-root constants; fresh phase-specific pre/post quiescence and inventory records with dedicated journals/predecessors; exact directory/empty-directory equality; and strict UTF-8 Unicode NFC collision keys. Those corrections are now written and await re-review; no toolkit/live action occurred.
- The final Terra re-review accepted state plan SHA-256 `f99b243a757d6448fe873bb37c64a4e082db63e693d4156d5d8f75cabdef7f10` with no Critical or Important issue. The handoff fixes the code GO, pre evidence capture, mandatory second evidence-bound GO before owner work, post capture/compare, and final Luna GO. Synthetic state implementation may resume; live actions remain NO-GO.

### 2026-08-16 — Smoke/readiness cross-layer contract amended and accepted

- Independent review of the first state implementation exposed that a self-contained HMAC list of arbitrary message IDs could not prove raw retention, normalized provenance, Bridge acceptance, observer readiness/completeness, or actual reply causality. A parallel Terra review found the same missing seam in the smoke plan plus unbound owner-turn fields and underspecified crash recovery.
- The smoke plan now has one narrow state boundary: `compare_v2` accepts only a sealed successful `observer-close-v2` commitment and recursively authenticates an acyclic graph of fixed kinds. The graph covers ready state, exact raw Bridge artifacts, normalized sender/recipient/channel/account/event and observed timestamps/message/reply/mentions/direction/content commitments, expectation, durable intent/attempt, actual Bridge acceptance, optional inbound reply, exact event heads/count, and complete close. Unsolicited events retain legitimate reply provenance and are classified only by not replying to the accepted smoke message.
- Owner attestations use exact versioned host-turn statements/templates and a protected source-uniqueness journal; device-inventory fields are parsed from the exact host content and bound to the current-auth fingerprint. The smoke nonce is globally reserved in a private append-only journal before intent/attempt publication; every partial reservation is permanently burned and can never send on recovery.
- The plan explicitly avoids a close/terminal cycle: successful accepted-no-reply or inbound-reply outcomes end at the close; `external_effect_unknown` and `capture_failed` use failure-only terminal records and can never appear as successful closes. The common module will gain a fixed-kind protected raw-artifact verifier without exposing payload bytes.
- The revised plan SHA-256 is `8ac9a1cfb047e892c364e5516974e0699531b1ce81101fa1e40ed5c4fcb6e2f5`. After two correction rounds, the same Terra reviewer returned GO with no Critical or Important plan blocker. This authorizes synthetic implementation only; no owner turn or live runtime/auth/key/evidence/message/memory/network action is authorized.

### 2026-08-16 — Exact state-evidence module accepted

- Toolkit `f17ebe05598bf28c5f8a0f32dbc3b1d9dbb957a1` now proves exact preservation for every v1-scoped byte/path/metadata fact under full quiescence and verifies the separate sealed prep-03 evidence DAG without allowing observer evidence to mutate or excuse any legacy root.
- Correction commits closed production-compatible `0755`/`0644` read modes while rejecting group/world writes, Python-default legacy JSON escaping, held-FD rewind, strict independently validated inventories, keyed incident-global state nonces with crash recovery, and the full quarantine receipt -> phone-ready owner turn -> canonical device inventory -> changed current-auth -> fixed smoke expectation -> intent/attempt/acceptance -> raw/normalized event -> successful close graph.
- Full protected provenance covers sender, recipient, channel/account, event/observed timestamp, message, reply, mentions, direction/relation, media/content commitment, and raw artifact. Legitimate replies to unrelated messages remain unsolicited with their reply provenance intact; outbound observer events remain retained and counted. Unknown/capture-failed states and the retired self-contained ID list are mismatches.
- State authenticates common fixed-kind protected records and does not impose a private observer journal; prep-03 owns observer and one-shot durability. Final prep-02 plan SHA-256 is `8ed9fc0b8c7a796399db22700511d75ae16ce7e1b7d29c9a24580f8480865086`; prep-03 remains `8ac9a1cfb047e892c364e5516974e0699531b1ce81101fa1e40ed5c4fcb6e2f5`.
- Independent Terra verdict: ACCEPT, no Critical or Important finding. Reviewer verification passed 249 tests in 12.23 seconds plus Ruff/diff/status; controller verification passed 249 tests in 12.52 seconds plus Ruff/diff/status. No live/runtime/key/evidence/auth/service/network/QR/message/memory/archive/receipt action occurred. The next bounded task is synthetic smoke/readiness implementation; owner and production actions remain NO-GO.
