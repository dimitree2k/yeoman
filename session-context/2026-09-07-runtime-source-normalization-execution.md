...
## Post-deploy systemd sandbox correction

- The first PID guard correctly prevented a Gateway-managed Bridge spawn beside the active `yeoman-bridge.service`, but its readiness wait used `lsof` PIDs as the only listener signal. Under the Gateway unit's `ProtectSystem=strict`/bind sandbox, `lsof` cannot see peer-process PIDs even though the TCP listener remains reachable; the Gateway therefore exited after the 30-second wait with `RuntimeError: systemd Bridge is active but did not open its listener`.
- The failure was reproduced read-only with a transient systemd unit carrying the production sandbox properties. Inside that unit, `lsof -nP -tiTCP:3001 -sTCP:LISTEN` returned no PID while `ss` showed `127.0.0.1:3001`; the exact Gateway traceback identified `WhatsAppRuntimeManager.start_bridge()` as the failing path. A direct foreground run with the daemon environment but without the sandbox stayed healthy, isolating the cause to PID visibility rather than Bridge availability.
- `WhatsAppRuntimeManager._wait_for_systemd_bridge()` now accepts a successful loopback TCP connection as readiness when peer PIDs are hidden. The stale PID-file guard remains fail-closed and removes a stored PID that is dead or not a Bridge process. Three focused tests cover stale PID cleanup, no duplicate spawn beside an active systemd unit, and reachable-port readiness without visible PIDs.
- The source was redeployed with `yeoman deploy`; the Gateway unit was started after removing only its stale socket. A fresh 45-second live check now shows `yeoman-gateway.service` active with `NRestarts=0`, the canonical `/home/dm/.yeoman/run/gateway.sock`, the active Bridge on port 3001, and no `whatsapp-bridge.pid` or zombie Bridge process. The Bridge remains systemd-owned.

## Fresh final verification (2026-09-08, Europe/Berlin)

- `uv run pytest tests/shared tests/gateway tests/overseer`: **910 passed** in 58.87 seconds; this includes the three systemd/PID visibility tests. `uv run ruff check .`, `git diff --check`, and `git diff --cached --check` passed.
- `npm test` in `packages/bridge/`: build and all **24 tests passed**.
- The active units are healthy: Gateway PID `2179809`, `NRestarts=0`; Bridge PID `2171787`, listener `127.0.0.1:3001`; Overseer and Pinchtab remain inactive. The canonical socket is present with mode `0600`, `/home/dm/.yeoman/run/whatsapp-bridge.pid` is absent, and `/home/dm/.yeoman/var/run/` is empty.
- `yeoman gateway status`, `yeoman channels bridge status`, and `yeoman memory status` all exit successfully. The latter reports `db_path=/home/dm/.yeoman/data/memory/memory.db`, `state_dir=/home/dm/.yeoman/data/memory/session-state`, and `wal_files=47`; a direct Unix-socket `{"cmd":"ping"}` returns `{"status":"ok","response":"pong"}`.
- Read-only `PRAGMA quick_check` remains `ok` for all six productive SQLite databases, and the canonical session-state target contains 47 Markdown files. The source tree has only the pre-existing unrelated working-tree modifications; the runtime tree has only the previously recorded A2A Hermes URL change in `config.json`.
