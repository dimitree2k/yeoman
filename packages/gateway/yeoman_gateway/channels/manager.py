    ...
    def _init_channels(self) -> None:
        ...
        if self.config.channels.whatsapp.enabled:
            try:
                ...
                if self.processing_signals is not None:
                    signal_setter = getattr(
                        self.channels["whatsapp"], "set_processing_signals", None
                    )
                    if signal_setter is not None:
                        signal_setter(self.processing_signals)
                if self.reaction_action is not None:
                    reaction_setter = getattr(
                        self.channels["whatsapp"], "set_reaction_action", None
                    )
        ...
        if self.config.channels.discord.enabled:
            try:
                from yeoman_gateway.channels.discord import DiscordChannel
        ...
        if self.config.channels.feishu.enabled:
            try:
                from yeoman_gateway.channels.feishu import FeishuChannel
