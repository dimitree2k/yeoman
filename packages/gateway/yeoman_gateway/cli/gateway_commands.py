...
def _gateway_log_path() -> Path:
    from yeoman_shared.utils.helpers import get_logs_path
...
def _is_yeoman_gateway_command(cmd: str) -> bool:
    ...
    if "-m" in tokens:
        i = tokens.index("-m")
        if i + 1 < len(tokens) and tokens[i + 1] in ("yeoman.cli.commands", "yeoman_gateway.cli.commands"):
            return True
...
def _start_gateway_daemon(port: int, verbose: bool, ensure_whatsapp: bool = True) -> None:
    ...
    cmd = [sys.executable, "-m", "yeoman_gateway.cli.commands", "gateway", "--port", str(port)]
    if verbose:
        cmd.append("--verbose")
