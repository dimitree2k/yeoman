...
The bus transports `Message` directly. Drop `InboundMessage` — it's an unnecessary intermediate class. Yeoman is single-process, the bus is just an `asyncio.Queue` between coroutines, no serialization boundary exists.
