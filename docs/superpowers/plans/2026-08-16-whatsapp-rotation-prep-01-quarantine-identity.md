...
- Choose transaction nonce before artifact streaming and derive its deterministic artifact identity. After protected artifact streaming and immediately before PREPARED journal/exchange, rewalk/re-HMAC the canonical auth tree and require equality to the artifact canonical HMAC plus identical root/path inode/stat and held descriptor identity. The first durable PREPARED journal and every later transaction-journal state bind the exact authenticated `authorization_gate`; its recursively authenticated predecessors bind revocation and quiescence. Crash/rerun inspects nonce artifact, journal, bound gate, and exchanged identities: before any recovery action it recursively verifies the three supplied caller commitments and requires exact equality to the journal-bound chain. It may reuse a verified unchanged nonce artifact or record protected abandoned state, may finish cleanup after verified exchange, and never exchanges/quarantines twice or resumes under a different otherwise-valid gate chain.
...
- `verify_quarantine_receipt(commitment: EvidenceCommitment) -> VerifiedQuarantineReceipt` (fixed production configuration; an explicit private-config seam is synthetic-test-only)
...
`prepare_empty_private_sibling` creates and revalidates a held-parent empty current-user `0700` sibling. Before exchange, publish/fsync a protected `PREPARED` transaction journal binding nonce, deterministic artifact identity/commitment, canonical old-auth HMAC, active/replacement inode identities, and the exact authorization gate head. Every later journal state preserves the same gate head. `renameat2_exchange(..., RENAME_EXCHANGE)` atomically exchanges the held siblings; unsupported Linux/filesystem behavior fails precommit and preserves original auth. Immediately fsync the held parent; re-identify both names/held inodes so the exact sibling containing old auth is known before no-follow recursive removal. Fsync failure or post-exchange receipt failure is `failed_post_commit`: Bridge remains stopped, active auth remains empty, and rerun is cleanup-only after journal/gate/identity inspection. Failure or proof that the exact sibling remains likewise never returns success. There is no restore API.

The `quarantined` receipt has one exact schema: phase, transaction nonce, canonical serialization identifier, old-auth canonical HMAC, artifact commitment, and `authorization_gate`, which is the exact authenticated `owner-quarantine-authorized-v1` commitment consumed by this transaction. That authorization predecessor-links the exact authenticated `owner-all-devices-revoked-v1` commitment, whose predecessor is the exact verified quiescence commitment consumed by this transaction. `_verify_gates` requires the fixed source text and fields for both owner turns, distinct sources, and this exact chain before retaining the authorization gate head for success publication. Crash recovery receives and re-verifies the same three caller commitments and refuses to publish or reuse a success receipt whose bound gate head differs.

`verify_quarantine_receipt` authenticates the receipt and uses the already-available common fixed-kind opaque-artifact verifier on its `old-auth-quarantine-v1` commitment, validates phase/nonce/serialization/HMAC, then recursively verifies `quiescence -> revocation -> authorization -> receipt`, including the fixed source text, fixed fields, distinct owner sources, and exact predecessor commitments. It returns only an internal typed result and never prints or returns artifact plaintext. A generic same-kind protected record with a missing, swapped, malformed, or fabricated gate link is invalid. Add tests for each such substitution, for a valid receipt/gate with a missing, swapped-kind, or tampered old-auth artifact, and for a recovered transaction presented with a different otherwise-valid gate chain.
