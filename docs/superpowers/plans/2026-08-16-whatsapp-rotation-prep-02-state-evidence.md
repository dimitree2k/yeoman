...
The protected incident observer evidence is compared separately and is never represented as a v1-declared runtime entry. The comparator accepts only `whatsapp-rotation-observer-close-v2`, then recursively authenticates common fixed-kind prep-03 `EvidenceCommitment` records: ready; exact raw artifacts; contiguous normalized events with ready/prior-event links and complete channel/account, event/observed time, sender/recipient/message/reply/mentions/direction/relation/media/content provenance; expectation -> intent -> attempt -> Bridge acceptance; optional inbound reply; and the final acyclic close. State does not invent or require a private observer journal; prep-03 owns its observer and one-shot durability protocols. It requires protocol v3 authenticated readiness, one capture boundary, matching account, exact first/final heads and event count, fixed-kind raw-artifact verification, ordered capture/event timestamps, request/destination/message acceptance bindings, full-window completeness, and terminal state exactly `accepted_no_reply` or `inbound_reply_observed`. `external_effect_unknown`, `capture_failed`, wrong-kind records, the retired self-contained `whatsapp-rotation-observer-chain-v1`, plain decoded records, and fabricated or swapped links are mismatches. Expected inbound is derived only from reply-to equality with the authenticated accepted message; another legitimate reply remains unsolicited with its reply provenance intact. Authenticated outbound events remain retained and counted, and any nonzero exact event count yields classified-addition status. Inventory is encrypted evidence; public output reports fixed phase, status, commitments, and aggregate counts only.
...
First authenticate and load the exact fixed pre/post protected kinds, final keyed nonce reservations, and dedicated journals. Require distinct commitments/nonces, correct phase order, fresh independently authenticated quiescence receipts, post-to-pre predecessor binding, the same legacy plaintext SHA and verified-v1 flag, strict inventory invariants, then exact directory/file entry-set and per-entry fact equality. Reject same/swapped commitments, wrong kinds, generic protected records, and plain decoded inputs. Any addition inside a v1 root is mismatch. Accept only the sealed successful `whatsapp-rotation-observer-close-v2` DAG defined above and in the accepted prep-03 plan; classify after complete recursive verification. As part of that recursion, authenticate `quiescence -> owner-all-devices-revoked-v1 -> owner-quarantine-authorized-v1 -> auth-quarantine-receipt-v1 -> phone-ready-v1`, including the receipt-bound authorization gate head, fixed owner content and fields, and exact predecessors; missing, swapped, malformed, or generic same-kind gate links are mismatch. Never delete, normalize, open, rewrite, or replay a v1-root file.
